...
\begin{abstract}
On the one hand, deep neural networks (DNNs) have found their way through critical applications such as medical diagnosis and autonomous vehicles, where safety and security are required.  On the other hand, they benefit from their numerous parameters stored in memory systems which are prone to hardware related security and safety issues. In this work, we show that the recent generation of Rowhammer attacks can be leveraged to undermine accuracy of DNNs. Furthermore, we investigate both attack and defense as a search problem where the objective is to retrieve the most influential parameters out of the huge number of model parameters. 
\end{abstract}
