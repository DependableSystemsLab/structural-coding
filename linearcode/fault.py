...
def inject_memory_fault(model, config):
    ...
    if isinstance(config['flips'], str):
        if config['flips'].startswith('rowhammer'):
            ber = float('-'.join(config['flips'].split('-')[1:]))
            for _ in range(rnd.randint(7, 60)):
                start = rnd.randint(0, chunks - 1) * _2KB
                count = rnd.binomial(_2KB, ber)
                initial_count = len(bit_indices_to_flip)
                while len(bit_indices_to_flip) < count + initial_count:
                    bit_indices_to_flip.add(rnd.randint(start, start + _2KB - 1))
        elif config['flips'] == 'word':
            granularity = _2B
            start = rnd.randint(0, words - 1) * _2B
            bit_indices_to_flip.add(start)
        elif config['flips'] == 'column':
            number_of_corrupted_chunks = rnd.binomial(chunks, 1 / 128)
            corrupted_chunk_starts = set()
            while len(corrupted_chunk_starts) < number_of_corrupted_chunks:
                corrupted_chunk_starts.add(rnd.randint(0, chunks - 1) * _2KB)
            start = rnd.randint(0, _2KB // _2B - 1) * _2B
            ...
            number_of_corrupted_chunks = rnd.binomial(chunks, 1 / 128)
            while len(bit_indices_to_flip) < number_of_corrupted_chunks:
                bit_indices_to_flip.add(rnd.randint(0, chunks - 1) * _2KB)
            granularity = _2KB
        elif config['flips'] == 'chip':
            number_of_corrupted_chunks = rnd.binomial(chunks, 1 / 16)
            while len(bit_indices_to_flip) < number_of_corrupted_chunks:
                bit_indices_to_flip.add(rnd.randint(0, chunks - 1) * _2KB)
            granularity = _2KB
...
if __name__ == '__main__':
    ...
    assert len(indices) > 0
    indices, _ = inject_memory_fault(module, {'quantization': False, 'injection': 0, 'flips': 'rowhammer-0.0003'})
    indices = sorted(indices)
    corrupted_chunk_indices = set(i // _2KB for i in indices)
    assert len(corrupted_chunk_indices) > 0
    for i in indices:
        offset = min(abs(i - chunk_index * _2KB) for chunk_index in corrupted_chunk_indices)
        assert offset < _2KB, offset
